<#
.SYNOPSIS
    Steam Game Downloader - Downloads Games Thanks To YagiSheep

.DESCRIPTION
    YagiSheep Is The Best Hacker From Since 1 Year Of Expirence
    YagiSheep ALso A Best Coder With MORE THAN 2 YEARS OF EXPIRENCE.
    OTHER THINGS TALK TO YAGI FROM https://instagram.com/yagisheepofficial
...
$Host.UI.RawUI.WindowTitle = "GGCART BY YAGISHEEP (For GGCART-LAUNCHER)"
...
function Write-Header {
    ...
    if ($Mode -eq "github+morrenus") {
        $sourceLink = "$esc]8;;https://discord.gg/FJc3neAMPB$esc\ggcart$esc]8;;$esc\"
        $sourcePad  = "          "
    } elseif ($Mode -eq "github+manifesthub") {
        $sourceLink = "$esc]8;;https://discord.gg/FJc3neAMPB$esc\ggcart$esc]8;;$esc\"
        $sourcePad  = "       "
    } else {
        $sourceLink = "$esc]8;;https://discord.gg/FJc3neAMPB$esc\DISCORD$esc]8;;$esc\"
        $sourcePad  = "    "
    }
...
if ($env:MANIFEST_MODE) {
    ...
    Write-Host ""
    Write-Host "    1. YagiSheep GGCART kEY    (1 TIME ONLY USE , GET KEY FROM YAGISHEEP)" -ForegroundColor White
    Write-Host ""
    do {
        $modeChoice = Read-Host "  Enter choice (1)"
    } while ($modeChoice -notin @("1"))
...
if (-not $AppId) {
    $AppId = Read-Host "  Enter The Key YagiSheep Gives"
}
